...
* **元数据提取**: 精确提取标题、URL、添加时间及来源指纹。

### ⚙️ 转化 (Process)
* **结构归一化**: 抹平不同浏览器的格式差异，建立统一的内部数据模型。
* **物理隔绝**: 数据仅在本地 SQLite/JSON 中流转，这是一个真正的“飞行模式”应用。

### 📊 显现 (Reveal)
* **Chronos (时间流)**:
    * 回溯你的“兴趣编年史”，按月/年维度生成热力图。
    * 精准识别你的“高昂期”与“沉寂期”。
* **Themes (主题聚类)**:
    * **长程兴趣漂移检测 (Interest Drift Detection)**: 自动识别跨越数年的核心关注点（如“深度学习”、“装修灵感”），发现你的知识孤岛。
    * **本地语义提取**: 基于本地分词引擎与 TF-IDF 权重计算，无需 AI 介入即可完成高精度聚类。
* **Visualization**: 交互式时间轴与动态兴趣星群图。
